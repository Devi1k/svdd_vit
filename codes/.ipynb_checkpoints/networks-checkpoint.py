...
class PositionClassifier(nn.Module):
    def __init__(self, K, D, class_num=8):
        ...
        # self.pos_embed = nn.Parameter(torch.zeros(D, D, 1, 1))
    ...
    def infer(c, enc, batch):
        ...
        for x2_ in x2s:
            ...
            logits = c(h1, h2)
            # ys [64,]
            # print(y_t[i].view(64, -1).squeeze().size())
            loss = xent(logits, y_t[i].view(k, -1).squeeze())
            i += 1
            ls += loss
        return ls
    ...
    def forward(self, h1, h2):
        # [64,64,1,1]->[64,64]
        # h1 = h1 + self.pos_embed
        h1 = h1.view(-1, self.D)
        # h2 = h2 + self.pos_embed
        h2 = h2.view(-1, self.D)
